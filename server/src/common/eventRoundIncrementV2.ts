 ...
 * @constructor
 */
export const EventIncrementRoundV2 = async function (req: Request, eventId: string, userId: string, roundNumber: number) {
    ...
    logger.info(`roundNumber ${roundNumber} 'event.CurrentRound.RoundNumber', ${event.CurrentRound && event.CurrentRound.RoundNumber}`);
};

async function closeRound(event: EventDocument, userId: string) {
...
async function startRound(event: EventDocument, userId: string) {
